        ...
        @method('PUT') <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 30px;">
            <div>
                <div style="margin-bottom: 20px;">
                    ...
                    <input type="text" name="title" value="{{ $service->title }}" required style="width: 100%; padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px;">
                </div>

                <div style="margin-bottom: 20px;">
                    <label style="display: block; font-weight: 600; margin-bottom: 8px;">Mô tả ngắn</label>
                    <textarea name="summary" rows="3" style="width: 100%; padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px;">{{ $service->summary }}</textarea>
                </div>

                <div style="margin-bottom: 20px;">
                    <label style="display: block; font-weight: 600; margin-bottom: 8px;">Nội dung chi tiết</label>
                    <textarea id="editor" name="description" style="width: 100%; min-height: 250px;">{{ $service->description }}</textarea>
                </div>
            </div>

            <div style="background: #f8fafc; padding: 20px; border-radius: 10px; border: 1px solid #e2e8f0;">
                <div style="margin-bottom: 20px;">
                    <label style="display: block; font-weight: 600; margin-bottom: 8px;">Icon hiện tại: <i class="fa-solid {{ $service->icon }}"></i></label>
                    <input type="text" name="icon" value="{{ $service->icon }}" placeholder="fa-helmet-safety" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px;">
                </div>
